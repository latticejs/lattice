...
describe('Integration', () => {
  describe('Environment variables', () => {
    it('file env variables', async () => {
      ...
      expect(doc.getElementById('feature-file-env-original-2').textContent).to.equal(
    ...
    it('PUBLIC_URL', async () => {
      ...
      expect(doc.querySelector('head link[rel="shortcut icon"]').getAttribute('href')).to.equal(
        `${prefix}/favicon.ico`
      );
    });
    ...
    it('expand .env variables', async () => {
      ...
      expect(doc.getElementById('feature-expand-env-1').textContent).to.equal('basic');
      expect(doc.getElementById('feature-expand-env-2').textContent).to.equal('basic');
      expect(doc.getElementById('feature-expand-env-3').textContent).to.equal('basic');
      expect(doc.getElementById('feature-expand-env-existing').textContent).to.equal('fromtheshell');
    });
  });
});
