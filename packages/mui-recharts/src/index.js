...
export default (WrappedChart) => {
  ...
  return withStyles(styles, { withTheme: true })(class extends Component {
    static displayName = `MuiRechart(${WrappedChart.displayName || WrappedChart.name})`

    applyStyleFor(element) {
      const { theme } = this.props;
      const { type: { displayName } } = element; // TODO: check this in prod build

      switch (displayName) {
        case 'Tooltip':
          const { props: { wrapperStyle } } = element;
          return {
            wrapperStyle: {
