...
const employees = generator.employees();
const departments = generator.departments();
const sales = generator.sales();
const activities = generator.activities(users[0], employees, 100);
const stats = [
  {
    label: 'Followers',
    unit: 'New Followers',
...
module.exports = {
  passwords: users.map(u => ({ email: u.email, password: u.password })),
  db: {
    users: users.map(u => ({ id: u.id, name: u.name, email: u.email, teams: u.teams })),
    employees,
    departments,
    activities,
    sales,
